...
fn process_continuous_chunk_payload(
    chunk_bounds: &RasterChunk,
    ...
    chunk_stride: u32,
    nodata: Option<f64>,
) -> Vec<HashMap<u64, H3Accumulator, FxBuildHasher>> {
    let mut chunk_maps = Vec::with_capacity(resolutions.len());
    for _ in 0..resolutions.len() {
        chunk_maps.push(HashMap::with_hasher(FxBuildHasher::default()));
    }

    match decoding_result {
        DecodingResult::U8(slice) => {
            let nd = nodata.and_then(|v| if (0.0..=255.0).contains(&v) { Some(v as u8) } else { None });
            process_continuous_slice_into_maps(slice, chunk_bounds, |x| x as f64, nd, resolutions, crs_transformer, gt, sampling, bbox, chunk_stride, nodata, &mut chunk_maps);
        }
        ...
        DecodingResult::F64(slice) => {
            let nd = nodata;
            process_continuous_slice_into_maps(slice, chunk_bounds, |x| x, nd, resolutions, crs_transformer, gt, sampling, bbox, chunk_stride, nodata, &mut chunk_maps);
        }
    }
...
impl MultiScanHorizonStreamer {
    ...
    pub fn new(reader: GeoTiffStreamReader, config: &MultiResolutionConfig) -> Result<Self> {
        ...
        let prefetcher = PrefetchedChunkReader::spawn(reader, chunk_indices, 64);
        let num_res = resolutions.len();
    ...
    pub fn fetch_next_batch(&mut self, max_rows: usize) -> Vec<MultiContinuousRecord> {
        while self.completed_buffer.len() < max_rows && !self.is_finished {
            ...
            // Parallel process all chunks in the batch across all available CPU cores with Rayon
            let parallel_results: Vec<(f64, Vec<HashMap<u64, H3Accumulator, FxBuildHasher>>)> = chunk_items
                .into_par_iter()
                .filter_map(|item| {
                    match item {
                        Ok((_chunk_idx, chunk_bounds, decoding_result)) => {
                            let bottom_lat = compute_chunk_bounds_bottom_lat(&chunk_bounds, gt, crs_transformer);
                            let maps = process_continuous_chunk_payload(
                                &chunk_bounds,
                                &decoding_result,
                                resolutions,
                                crs_transformer,
...
fn process_categorical_chunk_payload(
    chunk_bounds: &RasterChunk,
    decoding_result: &DecodingResult,
    resolutions: &[Resolution],
    crs_transformer: &CrsTransformer,
    ...
    chunk_stride: u32,
    nodata: Option<f64>,
) -> Vec<HashMap<u64, CategoricalAccumulator, FxBuildHasher>> {
    let mut chunk_maps = Vec::with_capacity(resolutions.len());
    for _ in 0..resolutions.len() {
        chunk_maps.push(HashMap::with_hasher(FxBuildHasher::default()));
    }
    ...
    match decoding_result {
        DecodingResult::U8(slice) => {
            let nd = nodata.and_then(|v| if (0.0..=255.0).contains(&v) { Some(v as u8) } else { None });
            process_categorical_slice_into_maps(slice, chunk_bounds, |x| Some(x as i64), nd, resolutions, crs_transformer, gt, sampling, bbox, chunk_stride, nodata, &mut chunk_maps);
        }
        DecodingResult::U16(slice) => {
            let nd = nodata.and_then(|v| if (0.0..=65535.0).contains(&v) { Some(v as u16) } else { None });
            process_categorical_slice_into_maps(slice, chunk_bounds, |x| Some(x as i64), nd, resolutions, crs_transformer, gt, sampling, bbox, chunk_stride, nodata, &mut chunk_maps);
        ...
        DecodingResult::U64(slice) => {
            let nd = nodata.and_then(|v| if v >= 0.0 { Some(v as u64) } else { None });
            process_categorical_slice_into_maps(slice, chunk_bounds, |x| if x <= i64::MAX as u64 { Some(x as i64) } else { None }, nd, resolutions, crs_transformer, gt, sampling, bbox, chunk_stride, nodata, &mut chunk_maps);
        }
        DecodingResult::I8(slice) => {
            let nd = nodata.and_then(|v| if (-128.0..=127.0).contains(&v) { Some(v as i8) } else { None });
            process_categorical_slice_into_maps(slice, chunk_bounds, |x| Some(x as i64), nd, resolutions, crs_transformer, gt, sampling, bbox, chunk_stride, nodata, &mut chunk_maps);
        ...
        DecodingResult::I64(slice) => {
            let nd = nodata.map(|v| v as i64);
            process_categorical_slice_into_maps(slice, chunk_bounds, |x| Some(x), nd, resolutions, crs_transformer, gt, sampling, bbox, chunk_stride, nodata, &mut chunk_maps);
        }
        DecodingResult::F32(slice) => {
            let nd = nodata.map(|v| v as f32);
            process_categorical_slice_into_maps(slice, chunk_bounds, |x| if x.is_finite() { Some(x.round() as i64) } else { None }, nd, resolutions, crs_transformer, gt, sampling, bbox, chunk_stride, nodata, &mut chunk_maps);
        }
        DecodingResult::F64(slice) => {
            let nd = nodata;
            process_categorical_slice_into_maps(slice, chunk_bounds, |x| if x.is_finite() { Some(x.round() as i64) } else { None }, nd, resolutions, crs_transformer, gt, sampling, bbox, chunk_stride, nodata, &mut chunk_maps);
...
impl MultiCategoricalHorizonStreamer {
    ...
    pub fn fetch_next_batch(&mut self, max_rows: usize) -> Vec<MultiCategoricalRecord> {
        while self.completed_buffer.len() < max_rows && !self.is_finished {
            ...
            // Parallel process all chunks in the batch across all available CPU cores with Rayon
            let parallel_results: Vec<(f64, Vec<HashMap<u64, CategoricalAccumulator, FxBuildHasher>>)> = chunk_items
                .into_par_iter()
                .filter_map(|item| {
                    match item {
                        Ok((_chunk_idx, chunk_bounds, decoding_result)) => {
                            let bottom_lat = compute_chunk_bounds_bottom_lat(&chunk_bounds, gt, crs_transformer);
                            let maps = process_categorical_chunk_payload(
                                &chunk_bounds,
                                &decoding_result,
                                resolutions,
                                crs_transformer,
