...
fn main() {
    ...
    println!("Test Dataset: {} ({} x {} = {:.2} Mpx)", cfl_path, width, height, total_pixels / 1_000_000.0);
    ...
    let patterns = [
        ...
        ("Gaussian 5-Point", SamplingPattern::gaussian_five_point(), 5),
        ("Hex 7-Point", SamplingPattern::hex_seven_point(), 7),
        ("16-Point Grid", SamplingPattern::sixteen_point(), 16),
    ];

    println!("\n{:<30} | {:<10} | {:<10} | {:<12} | {:<15}", "Sampling Pattern", "Points/Px", "Time", "Throughput", "Efficiency vs Center");
    println!("{:-<30}-|-{:-<10}-|-{:-<10}-|-{:-<12}-|-{:-<15}", "", "", "", "", "");
    ...
    for (name, pattern, spp) in &patterns {
        ...
        if spp == 1 {
            ...
            println!("{:<30} | {:<10} | {:<10.2?} | {:<8.2} Mpx/s | {:<15}",
            ...
            println!("{:<30} | {:<10} | {:<10.2?} | {:<8.2} Mpx/s | {:<5.1}% of Center",
    ...
    if let Ok(reader_probe_lf) = GeoTiffStreamReader::open(lf_path) {
        let lf_pixels = (reader_probe_lf.metadata.width as f64) * (reader_probe_lf.metadata.height as f64);
        println!("{:<30} | {:<10} | {:<10} | {:<12} | {:<15}", "Sampling Pattern", "Points/Px", "Time", "Throughput", "Efficiency vs Center");
        println!("{:-<30}-|-{:-<10}-|-{:-<10}-|-{:-<12}-|-{:-<15}", "", "", "", "", "");
        ...
        for (name, pattern, spp) in &patterns {
            ...
            let t0 = Instant::now();
            let mut streamer = raster_h3::aggregator::multi_horizon::MultiCategoricalHorizonStreamer::new(reader, &cfg).unwrap();
            let mut total_hexes = 0;
            loop {
