...
use raster_h3::aggregator::multi_horizon::{
    MultiResolutionConfig, MultiScanHorizonStreamer, MultiCategoricalHorizonStreamer,
};
...
fn main() {
    ...
    println!("  • Baseline (compact = false) : {:.2?} | {} hexagons output", d_base, hexes_base);
    ...
    loop {
        let n = s2.drain_completed_into(4096, |_, rec| {
            hexes_compact += 1;
            if rec.resolution == 7 { res7_hexes += 1; }
        });
        if n == 0 { break; }
    }
    ...
    println!("  • Pushdown (min_mean >= 5.0) : {:.2?} | {} hexagons output ({:.1}% rows dropped)",
        d_pushdown, hexes_pushdown, (1.0 - (hexes_pushdown as f64 / hexes_base as f64)) * 100.0);
    ...
    println!("  • Baseline (compact = false) : {:.2?} | {} hexagons output ({:.2} Mpx/sec)",
        d_lf_base, hexes_lf_base, 256.54 / d_lf_base.as_secs_f64());
